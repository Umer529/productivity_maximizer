...
_, y_consistency = make_regression(n_samples=100, n_features=19, noise=0.1, random_state=45)
...
y_consistency = np.interp(y_consistency, (y_consistency.min(), y_consistency.max()), (0, 1))
...
# Extra placeholder model for experimentation; currently not used by runtime scheduling.
consistency_model = RandomForestRegressor(n_estimators=10, random_state=45)
consistency_model.fit(X, y_consistency)
...
joblib.dump(consistency_model, 'artifacts/study_consistency_model.pkl')
...
print("- study_consistency_model.pkl (unused experimental model)")
